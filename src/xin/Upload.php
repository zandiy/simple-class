...
class Upload
{
    ...
    function __construct(array $config = [])
    {
        ...
        $this->savePath = isset($config['savePath']) ? $config['savePath'] : '/static/upload/attach';
        $this->urlPath = isset($config['urlPath']) ? $config['urlPath'] : '/static/attach';
    ...
    private function file($file)
    {
        if (isset($file['error']) && $file['error'] != 0) {
            ...
            if ($this->autoName)
                ...
                $filename = $file['name'];
            $filePath = get_path() . '/' . $saveDir . $filename;
            ...
            return ['result' => true, 'file_path' => $urlPath];
        }
    }
